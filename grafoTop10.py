...
acima_media_centralidade = {
    e: p for e, p in centralidade_empresas.items() if p > media_centralidade}
acima_media_intermediacao = {
    e: p for e, p in intermediacao_empresas.items() if p > media_intermediacao}
acima_media_proximidade = {
    e: p for e, p in proximidade_empresas.items() if p > media_proximidade}
...
G_filtrado = G.subgraph(set(empresas_resultantes_acima) | set(
    df['ORGÃO SUPERIOR CONTRATANTE'].unique()))
...
G_final = G_filtrado.subgraph(set(top_10_empresas) | set(
    df['ORGÃO SUPERIOR CONTRATANTE'].unique()))
...
for angle, empresa in zip(angles, empresas):
    pos[empresa] = (np.cos(angle) * 0.8, np.sin(angle) * 0.8)

plt.figure(figsize=(12, 12))
nx.draw(G_final, pos, with_labels=True, node_size=500, node_color='skyblue', edge_color='gray',
        alpha=0.7, width=1.0, font_size=6, font_weight='bold', connectionstyle='arc3,rad=0.1')
